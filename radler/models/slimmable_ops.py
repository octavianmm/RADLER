...
class SlimmableConv2d(nn.Conv2d):
    ...
    def forward(self, input):
        ...
        weight = self.weight[: self.out_channels, : self.in_channels, :, :].to(
            input.device
        )
        if self.bias is not None:
            bias = self.bias[: self.out_channels].to(input.device)
        else:
            bias = self.bias
...
class SlimmableConvTranspose2d(nn.ConvTranspose2d):
    ...
    def forward(self, input):
        ...
        self.out_channels = self.out_channels_list[idx]
        self.groups = self.groups_list[idx]
        weight = self.weight[: self.in_channels, : self.out_channels, :, :].to(
            input.device
        )
        if self.bias is not None:
            bias = self.bias[: self.out_channels].to(input.device)
...
#         bias = self.bias
#         c = (
#             make_divisible(self.num_features_max * self.width_mult / self.ratio)
#             * self.ratio
#         )
